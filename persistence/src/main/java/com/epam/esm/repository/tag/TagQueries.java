...
public interface TagQueries {
    ...
    String GET_MOST_USED_TAG_OF_USER =
            ...
            "(select o.certificate_id from orders o where o.user_id = :userId)  group by ct.tag_id " +
            "having count(ct.tag_id) = (select distinct count(ct.tag_id) from certificate_tag ct " +
            "where ct.certificate_id in (select o.certificate_id from orders o where o.user_id = :userId) " +
